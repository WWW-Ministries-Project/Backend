    ...
    async setUser(uid, userid, name, password, role = 0, cardno = 0) {
        try {
            ...
            // Validate 'uid'
                ...
                if (isNaN(parsedUid) || parsedUid <= 0 || parsedUid > 3000) {
                    throw new Error('Invalid UID: It should be a number between 1 and 3000');
                }

                // Validate 'userid'
                if (userid.length > 20) {
                    throw new Error('Invalid User ID: It should not exceed 20 characters');
                }

                // Validate 'name'
                if (name.length > 32) {
                    throw new Error('Invalid Name: It should not exceed 24 characters');
                }

                // Validate 'password'
                // if (passwordpassword.length < 8) {
                //     throw new Error('Invalid Password: It should not be less than 8 characters');
                // }

                // Validate 'cardno'
                if (cardno.toString().length > 10) {
                    throw new Error('Invalid Card Number: It should not exceed 10 digits');
                }
