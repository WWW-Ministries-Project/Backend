...
class ZUDP {
      ...
      await new Promise((resolve, reject) => {
        ...
        this.socket.send(buf, 0, buf.length, this.port, this.ip, (err) => {
          if (err) {
            ...
            log(
              `[UDP][SEND_CHUNK_REQUEST] Error sending chunk request: ${err.message}`,
            );
            reject(err);
          } else {
  ...
  async getTime() {
    try {
      ...
      const response = await this.executeCmd(
  ...
  async getRealTimeLogs(cb = () => {}) {
    ...
    if (this.socket.listenerCount("message") === 0) {
      ...
      console.warn(
        "Multiple message listeners detected. Ensure only one listener is attached.",
      );
    }
  }
}
