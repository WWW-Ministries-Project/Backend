...
export class AtttendanceService {
    ...
    async getAttendanceForAllUsers(date: Date) {
        ...
        const allLogs = attendanceData.attendance.flatMap((device: { logs: { data: any; }; }) => device.logs.data);
      
        // Filter logs that match the date
        const filteredLogs = allLogs.filter((log: { record_time: string | number | Date; }) => {
        ...
        for (const user of allUsers) {
          ...
          logs.sort((a, b) => new Date(a.record_time).getTime() - new Date(b.record_time).getTime());
