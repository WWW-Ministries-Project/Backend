...
export class CourseController {
    ...
    async createCourse(req: Request, res: Response) {
        try {
          const newCourse= await courseService.createCourse(req.body.cohortId,req.body);
      ...
      async updateCourse(req: Request, res: Response) {
        try {
          const { id } = req.params;
          const updatedProgram = await courseService.updateCourse(Number(id), req.body);
          return res.status(200).json({ message: "Course updated", data: updatedProgram });
        } catch (error:any) {
