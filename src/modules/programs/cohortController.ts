...
export class CohortController {
  async createCohort(req: Request, res: Response) {
          try {
            ...
            const newCohort = await cohortService.createCohort(req.body.programId,req.body);
            return res.status(201).json({ message: "Cohort created", data: newCohort });
          } catch (error:any) {
        ...
        async getAllCohortsByProgramID(req: Request, res: Response) {
            try {
              ...
              const cohorts = await cohortService.getAllCohortsByProgramID(Number(req.params.id));
              return res.status(200).json({ data: cohorts });
            } catch (error:any) {
              return res.status(500).json({ message: "Error fetching cohort", error: error.message });
            }
          }
        ...
        async updateChorts(req: Request, res: Response) {
          try {
            const { id } = req.params;
            const updatedProgram = await cohortService.updateCohort(Number(id), req.body);
            return res.status(200).json({ message: "Cohort updated", data: updatedProgram });
          } catch (error:any) {
            return res.status(500).json({ message: "Error updating cohort", error: error.message });
          }
        }
        ...
        async deleteCohort(req: Request, res: Response) {
          try {
            ...
            return res.status(500).json({ message: "Error deleting cohort", error: error.message });
          }
        }
}
